...
void functions::connect_client(SOCKET listening)
{
	...
	while (true)
	{
		...
		if (FirewallAccepted == true)
		{
			...
			SQL_Query(buf);
			char* returnquery = const_cast<char*> (query.c_str());
...
int functions::callback(void* data/*data provided in 4th argument*/, int RowNum/*num collums in row*/, char** Fields/*feilds in row*/, char** ColName/*collum names*/)
{
	int i;
	if (query.size() == 0)
	{
...
void functions::SQL_Query(string userQuery)
{
	sqlite3* DB;
	int exit = 0;
...
{	
	...
	if (FirewallAccepted == true)
	{
		query += "your packet was accepted\n\n";
		cout << "packet was accepted\n" << endl;;
	}
	else if (FirewallAccepted == false)
	{
		query += "your packet was not accepted\n\n";
